...
exports.pushToken = {
    upsertToken: async (req, res) => {
        try {
            ...
            const [existingToken] = await connection_1.default.query("SELECT id FROM push_notif_token WHERE user_id = ? AND token = ?", [userId, token]);
            if (existingToken.length > 0) {
                // Token already exists, update the timestamp
                await connection_1.default.query("UPDATE push_notif_token SET updated_at = CURRENT_TIMESTAMP WHERE user_id = ? AND token = ?", [userId, token]);
                ...
                // Insert new token
                const [result] = await connection_1.default.query("INSERT INTO push_notif_token (token, user_id) VALUES (?, ?)", [token, userId]);
    ...
    getUserTokens: async (req, res) => {
        try {
            ...
            const [tokens] = await connection_1.default.query(`SELECT 
                    id,
                    token,
                    created_at,
                    updated_at
    ...
    deleteToken: async (req, res) => {
        try {
            ...
            const [result] = await connection_1.default.query("DELETE FROM push_notif_token WHERE id = ? AND user_id = ?", [tokenId, userId]);
    ...
    deleteTokenByValue: async (req, res) => {
        try {
            const { token } = req.body;
            const userId = req.user?.userId;
            if (!token || !userId) {
            ...
            const [result] = await connection_1.default.query("DELETE FROM push_notif_token WHERE token = ? AND user_id = ?", [token, userId]);
            if (result.affectedRows === 0) {
                return res.status(404).json({ message: 'Push notification token not found or not authorized' });
            }
            res.status(200).json({ message: 'Push notification token deleted successfully' });
